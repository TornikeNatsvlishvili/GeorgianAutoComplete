...
; (function($) {
    ...
    $.makeSuggest = function(area, suggests, options){
        ...
        $area.getSuggestions = function(word, all){
            lastWord = "";
            all_tokens = all.split(" ");
            if(all_tokens.length > 1) {
                lastWord = all_tokens[all_tokens.length - 2]  
            }
            
            return this.suggests(word, lastWord);
        }
        ...
        $area.getCompletion = function(performCycle) {
            ...
            var suggests = this.getSuggestions(text, this.val());
